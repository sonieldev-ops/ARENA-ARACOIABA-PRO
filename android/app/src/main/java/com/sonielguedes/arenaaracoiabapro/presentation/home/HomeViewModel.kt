    ...
    private fun loadHomeData() {
        viewModelScope.launch {
            publicRepository.observeActiveChampionships().collectLatest { championships ->
                ...
                if (activeChamp != null) {
                    combine(
                        athleteRepository.observeAllLiveMatches(),
                        athleteRepository.observeAllMatches(),
                        athleteRepository.observeRanking(activeChamp.id),
                        athleteRepository.observeTopScorers(activeChamp.id)
